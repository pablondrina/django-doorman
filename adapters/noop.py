...
Behavior:
- get_by_phone / get_by_email / get_by_uuid return a DoormanCustomerInfo
  with the input value deterministically mapped to a UUID.
- create_for_phone returns a DoormanCustomerInfo using the phone as identity.
...
class NoopCustomerResolver:
    ...
    DoormanCustomerInfo instances. No database or external service
