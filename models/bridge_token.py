...
def default_expiry():
    ...
    return timezone.now() + timedelta(minutes=doorman_settings.BRIDGE_TOKEN_TTL_MINUTES)
...
class BridgeToken(models.Model):
    ...
    Flow:
    1. Manychat/backend calls POST /doorman/bridge-tokens
    2. Receives URL with token
    3. Sends to customer
    4. Customer clicks -> GET /doorman/bridge/exchange?t=...
    5. Doorman validates, creates session, redirects
