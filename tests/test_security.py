"""
Security tests for Doorman (H06).
...
from doorman.conf import get_doorman_settings
from doorman.exceptions import GateError
from doorman.gates import Gates
from doorman.models import BridgeToken, MagicCode
from doorman.models.magic_code import generate_code, generate_raw_code, verify_code
from doorman.services.auth_bridge import AuthBridgeService
from doorman.services.verification import VerificationService
from doorman.utils import safe_redirect_url
from doorman.views.bridge import BridgeTokenCreateView, BridgeTokenExchangeView
...
class TestOTPGeneration:
    ...
    def test_otp_uses_secrets_module(self):
        ...
        import doorman.models.magic_code as mc_module
...
class TestBridgeTokenCreateAuth:
    ...
    def _post_create(self, data, headers=None):
        ...
        Mocks _build_url to avoid reverse() failing when doorman URLs
        ...
        request = factory.post("/doorman/bridge/create/", body, **kwargs)
        view = BridgeTokenCreateView.as_view()
        with patch.object(
            AuthBridgeService,
            "_build_url",
            return_value="https://test.local/doorman/bridge/?t=mock",
        ):
            return view(request)

    @override_settings(DOORMAN={"BRIDGE_TOKEN_API_KEY": TEST_API_KEY})
    ...
    @override_settings(DOORMAN={"BRIDGE_TOKEN_API_KEY": ""})
...
class TestBridgeTokenLifecycle:
    ...
    def test_invalid_token_does_not_redirect(self):
        ...
        factory = RequestFactory()
        request = factory.get("/doorman/bridge/", {"t": "invalid-token-123"})
        from django.contrib.sessions.backends.db import SessionStore
        ...
        with patch("doorman.views.bridge.render", return_value=None) as mock_render:
            view(request)
...
class TestPIILogging:
    ...
    def test_session_values_not_in_info_logs(self, customer, bridge_token):
        ...
        with patch("doorman.services.auth_bridge.logger") as mock_logger:
